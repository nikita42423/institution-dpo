			...
			<?php
				...
				foreach ($course as $row) {
					...
					if ($row['count_user'] >= $row['count_in_group'])
					{
						//занято
						$td1 = '<td colspan="';
						$td2 = '" class="table-danger text-danger p-0 text-center align-middle border-dark"><small>'.$row['count_user'].'</small></td>';
					...
					else
					{
						//свободно
						$td1 = '<td colspan="';
						$td2 = '" class="table-secondary p-0 text-center align-middle border-dark"><small>'.$row['count_user'].'<small></td>';
